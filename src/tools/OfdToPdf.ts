...
const OfdToPdfSchema = z.object({
  filePath: z.string().describe("OFD 文件的本地路径"),
});

export function registerOfdToPdf(server: McpServer) {
  server.registerTool('ofd_to_pdf', {
    description: '将本地 OFD 文件转换为 PDF 格式',
    inputSchema: OfdToPdfSchema,
  }, async (args: unknown) => {
    try {
      const { filePath } = OfdToPdfSchema.parse(args);
      
      Logger.info(`开始转换 OFD 文件: ${filePath}`);
      ...
      const request = buildPdfToOfdRequest(base64Str, fileName);
        ...
        if (retCode === 1) {
          ...
          Logger.info(`OFD 转换成功，PDF 文件地址：${fileUrl}`);
          return {
            content: [
              {
                type: "text" as const,
                text: `转换成功！PDF 文件地址：${fileUrl}`,
              },
            ],
          };
        } else {
          Logger.warn(`OFD 转换失败 [${retCode}]：${retMsg}`);
          return {
            content: [
        ...
        Logger.error(`OFD 转换异常：${error.message}`, error);
        return {
          content: [
